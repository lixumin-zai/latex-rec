...
def parse_option():
    ...
    parser.add_argument("--pretrained_model_name_or_path", default="./models/")
...
def run_nougat_latex():
    ...
    # batch test
    images = []
    for img_path in glob.glob("./public/*.png")[:]:
        image = Image.open(img_path)
        if not image.mode == "RGB":
            image = image.convert('RGB')
            images.append(image)
    ...
    st = time.time()
    pixel_values = latex_processor(images, return_tensors="pt").pixel_values
    task_prompt = tokenizer.bos_token
    decoder_input_ids = tokenizer([task_prompt]*len(images), add_special_tokens=False,
                                  return_tensors="pt").input_ids
    # print(pixel_values.shape, decoder_input_ids.shape)
    # decoder_input_ids = decoder_input_ids.expand(10, -1) 
    with torch.no_grad():
        outputs = model.generate(
            pixel_values.to(device),
            decoder_input_ids=decoder_input_ids.to(device),
